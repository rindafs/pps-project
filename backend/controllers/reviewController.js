...
exports.getReviewsByProductAndUser = (req, res) => {
  ...
  if (!product_id || !user_id) {
    return res.status(400).json({ message: 'Product ID dan User ID diperlukan' });
  }
...
exports.createReview = (req, res) => {
  ...
  if (!user_id || !product_id || !rating || !comment) {
    return res.status(400).json({ message: 'user_id, product_id, rating, dan comment harus diisi' });
  }
...
exports.updateReviewByUser = (req, res) => {
  ...
  Review.findByUserId(user_id, (err, review) => {
    if (err) {
      return res.status(500).json({ message: 'Terjadi kesalahan saat mencari review' });
    }
    if (!review) {
      return res.status(404).json({ message: 'Review tidak ditemukan untuk user ini' });
    }
    ...
    Review.updateByUserId(user_id, data, (err, result) => {
      if (err) {
        return res.status(500).json({ message: 'Terjadi kesalahan saat mengupdate review' });
      }
      if (result.affectedRows === 0) {
        return res.status(404).json({ message: 'Review tidak ditemukan atau tidak dapat diperbarui' });
      }
      res.json({
        id: review.id,
        user_id,
...
exports.deleteReviewByUserIdAndProductId = (req, res) => {
  ...
  Review.deleteByUserIdAndProductId(user_id, product_id, (err, result) => {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Review tidak ditemukan untuk user dan produk ini' });
    }
...
exports.getAverageRating = (req, res) => {
  ...
  Review.getAverageRating(product_id, (err, results) => {
    ...
    if (results.length === 0 || results[0].averageRating === null) {
      return res.status(404).json({ message: 'Produk tidak ditemukan atau belum ada rating' });
    }
    res.json({
      product_id,
