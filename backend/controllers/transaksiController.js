...
exports.getTransactionByUserId = (req, res) => {
  ...
  Transaction.getByUserId(user_id, (err, results) => {
    ...
    if (results.length === 0) {
      return res.status(404).json({ message: 'Transaksi tidak ditemukan untuk user_id ini' }); // Jika data tidak ditemukan
    }
...
exports.createTransaction = async (req, res) => {
  ...
  if (!user_id || !product_id || !jumlah) {
      return res.status(400).json({ message: 'User ID, Product ID, dan Jumlah harus diisi' });
  }
  ...
  try {
      ...
      Transaction.getAll((err, transactions) => {
          if (err) {
              return res.status(500).json({ message: 'Gagal memeriksa transaksi', error: err });
          }

          const existingTransaction = transactions.find(
              (t) => t.user_id === parseInt(user_id) && t.product_id === parseInt(product_id)
          );
          ...
          Transaction.create(transactionData, (createErr, result) => {
              if (createErr) {
                  return res.status(500).json({ message: 'Gagal menambahkan transaksi', error: createErr });
              }
...
exports.updateTransaction = (req, res) => {
  ...
  Transaction.update(id, transactionData, (err, result) => {
    ...
    res.json({ message: 'Transaksi berhasil diperbarui', id, ...transactionData });
  });
};
