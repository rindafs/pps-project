const db = require('../db/connection'); // Pastikan path ini sesuai dengan konfigurasi koneksi database Anda
...
Product.update = (id, data, callback) => {
  const { nama, harga, deskripsi, gambar } = data;
  const query = 'UPDATE products SET nama = ?, harga = ?, deskripsi = ?, gambar = ? WHERE id = ?';
...
Product.delete = (id, callback) => {
  ...
  db.beginTransaction((err) => {
    ...
    db.query('UPDATE reviews SET product_id = NULL WHERE product_id = ?', [id], (err) => {
      if (err) return db.rollback(() => callback(err));

      // Langkah 2: Update product_id menjadi NULL di tabel transactions
      db.query('UPDATE transactions SET product_id = NULL WHERE product_id = ?', [id], (err) => {
        if (err) return db.rollback(() => callback(err));
