...
Transaction.getByUserId = (user_id, callback) => {
  const query = 'SELECT * FROM transactions WHERE user_id = ?'; // Query untuk mengambil data
  db.query(query, [user_id], (err, results) => {
    if (err) {
      return callback(err, null); // Kembalikan error jika terjadi
    }
    callback(null, results); // Kembalikan hasil query
  });
};
